...
section('PERFORMANCE POR PAR');
{
  const pairs = [...new Set(closedTrades.map(t => t.pair))].sort();
  if (pairs.length === 0) {
    console.log('  Sem dados por par ainda.');
  } else {
    for (const p of pairs) {
      const rows = closedTrades.filter(t => t.pair === p);
      const wr = calcWinRate(rows);
      const pnl = rows.reduce((s, t) => s + (t.pnl ?? 0), 0);
      const pfPar = calcProfitFactor(rows);
      const wins = rows.filter(t => (t.pnl ?? 0) > 0).length;
      const losses = rows.filter(t => (t.pnl ?? 0) < 0).length;
      const pfStr = pfPar === Infinity ? '∞' : pfPar.toFixed(2);
      console.log(`  ${p.padEnd(10)}: ${bar(wr, 12)} ${pct(wr)} | ${rows.length} trades (${wins}W/${losses}L) | PnL ${usd(pnl)} | PF ${pfStr}`);
    }
  }
}
...
section('MELHOR E PIOR TRADE');
{
  ...
  const fmt = (t: TradeRow) => {
    ...
    const date = t.created_at.slice(0, 16).replace('T', ' ');
    return `${date} | ${t.pair} | ${t.action} | Entry: ${entry} | PnL: ${usd(t.pnl ?? 0)} | conf: ${(t.confidence * 100).toFixed(0)}%`;
  };
...
async function sendReportToTelegram(): Promise<void> {
  ...
  const pairs = [...new Set(closedTrades.map(t => t.pair))].sort();
  const parLines = pairs.map(p => {
    const rows = closedTrades.filter(t => t.pair === p);
    const wr = ((calcWinRate(rows)) * 100).toFixed(1);
    const pnl = rows.reduce((s, t) => s + (t.pnl ?? 0), 0);
    const pfPar = calcProfitFactor(rows);
    const pfParStr = pfPar === Infinity ? '∞' : pfPar.toFixed(2);
    const pnlStr = (pnl >= 0 ? '+' : '') + pnl.toFixed(2);
    return `${p}: ${rows.length}T | WR ${wr}% | PnL ${pnlStr} | PF ${pfParStr}`;
  }).join('\n');
  ...
  const text = [
    ...
    ``,
    `*Hoje*`,
    `Trades: ${todayTrades.length} | Win Rate: ${(todayWr * 100).toFixed(1)}% | PnL: ${(todayPnl >= 0 ? '+' : '') + todayPnl.toFixed(2)} USDT (${todayW}W/${todayL}L)`,
    ``,
    `*Por Par*`,
    parLines || 'Sem dados por par',
    ``,
    `*Por Confidence*`,
    tierLines,
    ``,
